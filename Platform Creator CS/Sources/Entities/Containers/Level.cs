    ...
    public class Level : EntityMatrixContainer {
        ...
        public void UpdateCamera(Camera2D camera, bool lerp) {
            ...
            if (FollowEntity != null) {
                var posX = MathHelper.Clamp(FollowEntity.Box.Center().X - viewportWidthZoom / 2f, MatrixRect.Left(),
                    MatrixRect.Right());
                var posY = MathHelper.Clamp(FollowEntity.Box.Center().Y - viewportHeightZoom / 2f, MatrixRect.Top(),
                    MatrixRect.Bottom());

                var lerpX = MathHelper.Clamp(MathHelper.Lerp(camera.Position.X, posX, 0.1f),
                    MatrixRect.Left(), MatrixRect.Right() - viewportWidthZoom);
                var lerpY = MathHelper.Clamp(MathHelper.Lerp(camera.Position.Y, posY, 0.1f),
        ...
        public static bool LoadFromFile(string levelDir, out Level level) {
            try {
                level = SerializationFactory.DeserializeFromFile<Level>(levelDir + $"/{Constants.LevelDataFile}");
